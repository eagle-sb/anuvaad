...
class CreateUser(Resource):
    ...
    def post(self):
        ...
        parser.add_argument('roles', type=str, location='json', help='Roles are required', required=True)
        ...
        if UserManagementRepositories.store(args['userID'], args['name'], args['userName'], args['password'], args['email'], args['phoneNo'], roles) == False:
            res = CustomResponse(Status.ERR_GLOBAL_MISSING_PARAMETERS.value, None)
            return res.getresjson(), 400
...
class UpdateUser(Resource):
    ...
    def post(self):
        ...
        if UserManagementRepositories.update(args['userID'], args['name'], args['userName'], args['password'], args['email'], args['phoneNo'], roles) == False:
            res = CustomResponse(Status.ERR_GLOBAL_MISSING_PARAMETERS.value, None)
            return res.getresjson(), 400
        res = CustomResponse(Status.SUCCESS.value, None)
        return res.getres()
